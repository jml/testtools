...
class MatchesSetwise(object):
    ...
    def match(self, observed):
        ...
        if not_matched or remaining_matchers:
            ...
            # XXX: zomg this is way too long.
            if len(not_matched) == 0:
                if len(remaining_matchers) > 1:
                    msg = _u("There were %s matchers left over: ") % (
                        len(remaining_matchers),)
                else:
                    msg = _u("There was 1 matcher left over: ")
                # XXX: Is `str` correct for Python 2 & 3?
                msg += _u(', ').join(map(str, remaining_matchers))
                return Mismatch(msg)
            elif len(remaining_matchers) == 0:
                if len(not_matched) > 1:
                    return Mismatch(
                        _u("There were %s values left over: %s") % (
                            len(not_matched), not_matched))
                else:
                    return Mismatch(
                        _u("There was 1 value left over: %s") % (
                            not_matched, ))
            else:
                ...
                if common_length > 1:
                    msg = _u("There were %s mismatches") % (common_length,)
                else:
                    msg = _u("There was 1 mismatch")
                if len(remaining_matchers) > len(not_matched):
                    extra_matchers = remaining_matchers[common_length:]
                    msg += _u(" and %s extra matcher") % (len(extra_matchers),)
                    if len(extra_matchers) > 1:
                        msg += _u("s")
                    msg += _u(': ') + _u(', ').join(map(str, extra_matchers))
                elif len(not_matched) > len(remaining_matchers):
                    extra_values = not_matched[common_length:]
                    msg += _u(" and %s extra value") % (len(extra_values), )
                    if len(extra_values) > 1:
                        msg += _u("s")
                    msg += _u(': ') + str(extra_values)
                return Annotate(
